...
def submit(token, post_url, order_id):
    ...
    try:
        ...
        if res.get('id'):
            ...
            print(f'Processing Order ID: {order_id} - Currently Sharing [{count_of_2}]', end='\r')
    except Exception as e:
...
def process_orders():
    print("Checking for new orders...")  # Print each time we are checking for new orders
    ...
    if get_orders:  # If there are orders, process them
        ...
        if not new_order_found:
            print("No New Pending Orders Yet - Checking every 10 seconds.")  # Print message if no new orders found
